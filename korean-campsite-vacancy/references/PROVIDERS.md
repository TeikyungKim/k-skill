...
동해시는 **이용일 30일 전 오전 11시**에 예약창을 연다. 그 전에도 상세 endpoint는 정상 응답하는데, 숫자가 **잔여 면수가 아니다.** 이걸 잔여로 읽으면 "전 시설 여유"라는 정반대 결론이 난다.
...
| (빈 값) | `not_open` | **예약창 미오픈. 숫자를 잔여로 읽지 않는다** (아래 연박 절 참고) |
...
### 연박은 미오픈 날짜까지 뻗는다 (2026-09-02 실측)

앞 절은 오랫동안 `not_open` 날짜의 숫자를 "총 정원"이라고 적었다. **틀렸다.** 미오픈 날짜의 숫자는 **연박 예약이 이미 먹은 뒤의 잔여**일 수 있다.

동해시는 하루치씩 순차로 창을 열지만, **연박 예약은 첫 밤이 열린 시점에 아직 열리지 않은 뒷날짜까지 한 번에 잡는다.** 즉 오픈 규칙은 "예약을 시작할 수 있는 날"을 정하는 것이고, 연박 길이를 제한하지 않는다.

2026-09-02 11:00에 `donghae-mangsang`의 10/2 이용분이 열렸다. 11:22에 같은 달을 조회한 결과다.

| 존 | 10/9 (미오픈, 기준선) | 10/4 (미오픈) | 10/3 (미오픈) | 10/2 (`full`) |
| --- | --- | --- | --- | --- |
| 전통한옥 | 24 | 15 | 3 | 마감 |
| 든바다 | 20 | 16 | 5 | 마감 |
| 난바다 | 14 | 13 | 8 | 마감 |
...
10/3과 10/4는 조회 시점에 **아무도 단독 예약할 수 없는 날짜**였다. 그런데도 10/9 기준선보다 크게 깎여 있다. 열린 날짜가 10/2뿐이었으므로 이 감소분은 **10/2에 입실해 두 밤·세 밤을 잡은 연박** 말고는 설명되지 않는다. 10/4까지 줄었다는 것은 3박 예약이 실제로 체결됐다는 뜻이다.

여기서 나오는 규칙 두 가지다.

1. **N박 연박의 승부는 첫 밤이 열리는 날 11:00에 끝난다.** 마지막 밤이 열릴 때까지 기다리면 앞 밤들이 이미 소진돼 있다. 금·토·일 3박이면 그 **금요일의 30일 전 11:00**이 유일한 기회다.
2. **`not_open` 숫자를 총 정원으로 단정하지 않는다.** 연박 소진 여부를 알려면 **충분히 먼 미개시 날짜를 기준선으로 함께 조회해 비교**한다. 조회 대상보다 최소 며칠 뒤, 연박이 닿을 수 없는 날짜를 고른다.

이 어댑터는 미오픈 날짜에 `available: False`를 그대로 유지한다. 기준선 비교는 **호출자가 날짜를 더 넘겨서** 하는 판단이고, 어댑터가 총 정원을 추정하지는 않는다.

같은 회차에 소진 속도도 실측했다. 망상 10/2는 11:18에 2면(전통한옥 1 · 자동차캠핑장 1), 11:20에 1면, **11:22에 `full`** 이었다. 오픈 22분 만에 전멸이다. 연휴 구간에서는 조회 결과의 유효시간이 분 단위라는 뜻이다.

### 산발적 `net::ERR_ABORTED`

`donghae` provider는 월 달력 이동(`Page.goto`)에서 `net::ERR_ABORTED`로 간헐 실패한다. 로그인은 성공한 뒤 발생하고, **같은 명령을 그대로 재실행하면 통과한다.** 2026-09-02 회차에서 `donghae-mangsang`이 2회 연속 실패 후 3회차에 성공했다. 어댑터에 재시도 로직은 없으므로 `fetch_failures`가 이 오류로 잡히면 **재시도를 한 번 더 해보고** 판정한다.
