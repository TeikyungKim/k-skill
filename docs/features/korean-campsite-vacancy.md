...
**예약창 오픈 여부를 반드시 함께 본다.** 동해시는 이용일 **30일 전 오전 11시**에 예약창을 연다. 그 전에도 조회는 되지만 아직 아무도 예약할 수 없어 **총 정원이 그대로** 돌아온다. 그래서 어댑터는 월 달력의 날짜별 라벨(`예약현황보기` / `예약마감` / `예약종료` / 빈 값)을 함께 읽어 `booking_status`를 붙인다. `open`이 아닌 날은 숨기지 않고 보여주되 예약 가능으로 집계하지 않고 경고 문구를 단다.
