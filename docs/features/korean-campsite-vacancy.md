...
레지스트리에서 가장 가벼운 경로다. 로그인도 브라우저도 파라미터도 없이 `GET /user/conn/campReserve.do` 한 번이면 서버사이드 렌더된 **두 달치(당월+익월)** 달력이 그대로 온다.

`div.area`가 예약 가능(`A구역 : 11`), `div.area_done`이 마감이다. 달력 옆에 월 캡션이 없어서 파서는 일자 시퀀스가 되감기는 지점(…31, 1, 2…)을 월 경계로 삼는다.

**2개월 창 밖 날짜는 실패로 명시 보고한다.** 조용히 비우면 "빈자리 없음"으로 오독되기 때문이다. 광명시민 추첨 + 그 외 선착순 혼합 운영이라 잔여 면수 해석에 주의가 필요하다.
...
**예약창 오픈 여부를 반드시 함께 본다.** 동해시는 이용일 **30일 전 오전 11시**에 예약창을 연다. 그 전에도 조회는 되지만 숫자가 잔여 면수가 아니다. 그래서 어댑터는 월 달력의 날짜별 라벨(`예약현황보기` / `예약마감` / `예약종료` / 빈 값)을 함께 읽어 `booking_status`를 붙인다. `open`이 아닌 날은 숨기지 않고 보여주되 예약 가능으로 집계하지 않고 경고 문구를 단다.

**미오픈 날짜의 숫자는 총 정원도 아니다.** 예약창은 하루치씩 열리지만 **연박 예약은 첫 밤이 열린 시점에 아직 미오픈인 뒷날짜까지 한 번에 잡는다.** 2026-09-02 11:00에 망상 10/2분이 열린 뒤 11:22 조회에서, 여전히 `not_open`인 10/3·10/4의 숫자가 먼 미개시 날짜(10/9) 대비 크게 깎여 있었다(자동차캠핑장 37 → 16 → 3). 열린 날짜가 10/2뿐이었으므로 그 감소분은 10/2 입실 연박이 먹은 것이다. 따라서 **N박 연박의 승부는 첫 밤이 열리는 날 11:00에 끝나고**, 미오픈 구간 숫자를 해석하려면 **연박이 닿을 수 없는 먼 미개시 날짜를 기준선으로 함께 조회**해야 한다. 어댑터는 총 정원을 추정하지 않고 판단을 호출자에게 남긴다.
