# realty-listing-search — assembled instructions
...
- Execute bundled helpers only through `npx -y @nomadamas/k-skill@0 exec realty-listing-search scripts/<file> -- <args>`; do not assume a repository-relative or installed-skill-relative path.
- Resolve an asset path with `npx -y @nomadamas/k-skill@0 path realty-listing-search <relative-path>` only when another tool explicitly requires a filesystem path.
- Read bundled references through `npx -y @nomadamas/k-skill@0 read realty-listing-search references/<file>`.
